...
async function makeImage({ one, two }) {
  ...
  batgiam_img.composite(circleOne.resize(235, 232), 511, 123).composite(circleTwo.resize(229, 225), 95, 126);
...
module.exports.run = async function ({ event, api, args }) {    
  ...
  if (!mention[0]) return api.sendMessage("ভালোবাসার মানুষ টি কে টেগ করুন-!!💏🙈", threadID, messageID);
      ...
      return makeImage({ one, two }).then(path => api.sendMessage({ body: "- 𝗧𝗵𝗶𝘀 𝗮𝗯𝗼𝘂𝘁 𝗹𝗶𝗻𝗲-!! 🖤🤍\nপ্রিয়__😽💚🌺\n\nতুমি ফুল হয়ে থেকে যাও আমি যত্ন করে রেখে দিব-!😘🤍🙃\n\n__🖤🍒", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
  }
    }
